...
const AdminProductEditPage: React.FC = () => {
    ...
    const addVariant = () => {
        const newVariants = [...(product?.variants || []), { id: `new-${Date.now()}`, name: '', photoCount: 1, price: 0 }];
        setProduct(prev => prev ? { ...prev, variants: newVariants } : null);
    };
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        {product.variants?.map((variant, index) => (
                            <div key={index} className="grid grid-cols-1 sm:grid-cols-4 gap-4 p-4 border rounded-md relative">
