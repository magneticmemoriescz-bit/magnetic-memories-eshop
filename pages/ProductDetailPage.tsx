...
const ProductDetailPage: React.FC = () => {
    ...
    // Special logic for individual magnets (excluding sheets)
    const isMagnetIndividual = product.id === 'photomagnets' && !selectedVariant?.id.includes('sheet');

    // Calculate required photos
    // If it's individual magnets, required photos = quantity (e.g., 5 magnets = 5 photos)
    // Otherwise (calendars, sheets, wedding), required photos is fixed per unit (or set by variant)
    const photoCount = isMagnetIndividual 
        ? quantity * (selectedVariant?.photoCount || 1)
        : (selectedVariant ? selectedVariant.photoCount : product.requiredPhotos);
    ...
    const handleVariantChange = (variant: ProductVariant) => {
        ...
        setUploadedPhotoInfo({ photos: [], groupId: null });
        setError(null);
        // Reset quantity to 1 when changing variant, just in case
        setQuantity(1); 
    }
                        ...
                        <form className="mt-6" onSubmit={(e) => e.preventDefault()}>
                                ...
                                <div className="mt-4">
                                    {isMagnetIndividual ? (
                                        <select 
                                            ...
                                            className="block w-full sm:w-32 pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-brand-purple focus:border-brand-purple sm:text-sm rounded-md"
                                        >
                                            {Array.from({ length: 100 }, (_, i) => i + 1).map(num => (
                                                <option key={num} value={num}>{num} ks</option>
                                            ))}
                                        </select>
                                    ) : (
                            ...
                            <div className="mt-10">
                                <h3 className="text-sm text-dark-gray font-medium">
                                    Nahrajte fotografie
                                    {isMagnetIndividual && <span className="font-normal text-gray-500 ml-2">(Vyžaduje se {photoCount} {photoCount === 1 ? 'fotka' : (photoCount >= 2 && photoCount <= 4) ? 'fotky' : 'fotek'})</span>}
                                </h3>
                                <div className="mt-4">
                                    <FileUpload 
                                        maxFiles={photoCount} 
