...
const ProductDetailPage: React.FC = () => {
    ...
    const inputClasses = "w-full py-3 px-4 bg-white rounded-xl border-2 border-gray-100 font-bold focus:ring-2 focus:ring-brand-purple focus:border-brand-purple outline-none placeholder-gray-400 text-sm transition-all shadow-sm";
    
    // Společné třídy pro tlačítka výběru
    const selectionBtnBase = "relative p-2.5 rounded-xl border-2 text-left transition-all h-full flex flex-col justify-center";
    ...
    return (
        <div className="bg-white min-h-screen pb-40">
                    ...
                    <div className="mt-10 lg:mt-0 space-y-10">
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                {product.variants?.map(v => (
                                    ...
                                    >
                                        <div className={`text-sm font-black uppercase leading-tight ${selectedVariant?.id === v.id ? 'text-white' : 'text-gray-900'}`}>
                                            {v.name}
                                            {isWedding && v.id === 'a6' && <span className="block text-[9px] font-bold normal-case mt-0.5 opacity-80"> nejběžnější</span>}
                                        </div>
                            ...
                            <p className="mt-3 text-[11px] font-black text-brand-purple uppercase tracking-widest px-1">
                                TIP: pokud vaše fotografie neodpovídá vybranému rozměru, nevadí, upravíme ji
                            </p>
                        </section>

                        {/* 2. POČET KUSŮ - SJEDNOCENO */}
                        <section>
                            <h2 className="text-sm font-black text-gray-800 uppercase tracking-widest mb-4">2. Počet kusů</h2>
                            <div className="bg-gray-50 rounded-3xl p-5 border border-gray-100 shadow-inner space-y-5">
                                <div>
                                    <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 px-1">Vlastní počet</h3>
                                    <div className="relative">
                                        <select 
                                            value={quantity} 
                                            onChange={(e) => setQuantity(parseInt(e.target.value))} 
                                            className="w-full h-[56px] bg-white border-2 border-gray-100 rounded-xl px-4 font-black text-base focus:ring-2 focus:ring-brand-purple focus:border-brand-purple outline-none appearance-none cursor-pointer transition-all hover:border-gray-200"
                                        >
                                ...
                                {!isCalendar && (
                                    <div>
                                        <h3 className="text-[10px] font-black text-brand-purple uppercase tracking-widest mb-2 px-1">Zvýhodněné sady</h3>
                                        <div className="grid grid-cols-3 gap-2">
                                            {(isPregnancy || isWedding ? [10, 20, 50, 100] : [9, 15, 30]).map(q => (
                                                <button 
                                                    key={q} 
                                                    onClick={() => setQuantity(q)} 
                                                    className={`${selectionBtnBase} text-center items-center py-4 ${quantity === q ? selectionBtnActive : selectionBtnInactive}`}
                                                >
                                                    <span className="text-sm font-black uppercase">{q === 15 ? '14+1' : q === 30 ? '28+2' : `${q} KS`}</span>
                                                    <span className={`text-xs font-black mt-1 ${quantity === q ? 'text-white/80' : 'text-brand-pink'}`}>{getSetPrice(q)} Kč</span>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                        ...
                        <section>
                            ...
                            {isWedding && (
                                <div className="grid grid-cols-2 gap-2 mb-6">
                                    <button onClick={() => setDesignMode('motif')} className={`py-3.5 rounded-xl border-2 font-black uppercase text-xs tracking-widest transition-all ${designMode === 'motif' ? 'bg-brand-purple border-brand-purple text-white shadow-md' : 'bg-white border-gray-100 text-gray-400'}`}>Náš motiv</button>
                                    <button onClick={() => setDesignMode('custom')} className={`py-3.5 rounded-xl border-2 font-black uppercase text-xs tracking-widest transition-all ${designMode === 'custom' ? 'bg-brand-purple border-brand-purple text-white shadow-md' : 'bg-white border-gray-100 text-gray-400'}`}>Vlastní motiv</button>
                                </div>
                            )}
                                ...
                                <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 mb-8">
                                    {motifs.map((url, idx) => (
                                        <button key={idx} onClick={() => setActiveMedia(url)} className="group flex flex-col items-center">
                                            ...
                                            <span className={`mt-1.5 text-[8px] font-black text-center uppercase tracking-wider transition-colors leading-tight ${activeMedia === url ? 'text-brand-purple' : 'text-gray-400'}`}>
                            ...
                            <div className="space-y-4 bg-white border border-gray-100 rounded-3xl p-6 shadow-sm">
                                {product.hasTextFields && !isCalendar && !(isWedding && designMode === 'custom') && (
                                    <div className="space-y-4">
                                        <h3 className="text-[11px] font-black text-brand-purple uppercase tracking-widest px-1">Doplňte text, který si přejete mít na oznámení.</h3>
                                        <div className="grid grid-cols-1 gap-3">
                                            ...
                                            <textarea placeholder="Speciální přání (font, barva textu atd...)" className={inputClasses + " resize-none h-24 text-base"} onChange={(e) => setCustomText(p => ({...p, msg: e.target.value}))} />
                                        </div>
                                    </div>
                                )}
                                ...
                                {!isInLove && (isMagnets || isCalendar || isMerch || isPregnancy || isWedding || designMode === 'custom') && (
                                    <FileUpload 
                                        ...
                                        labelHint={(isWedding && designMode === 'custom') 
                                            ? "zde nahrajte svou vlastní grafiku svatebního oznámení" 
                                            : isPregnancy ? "(např. ultrazvuk)" : isCalendar ? "(nahrajte 12 fotek)" : isWedding ? "(nahrajte vaši společnou fotku)" : undefined}
                                    />
                                )}
                            </div>
                        </section>
                        ...
                        {(isWedding || isPregnancy) && (
                            <section>
                                ...
                                <label className={`flex items-start space-x-4 p-5 rounded-3xl border-2 cursor-pointer transition-all shadow-sm ${directMailing ? 'bg-brand-purple/5 border-brand-purple' : 'bg-white border-gray-100 hover:border-gray-200'}`}>
                                    <input type="checkbox" checked={directMailing} onChange={() => setDirectMailing(!directMailing)} className="mt-1 h-6 w-6 text-brand-purple rounded border-gray-300 focus:ring-brand-purple" />
                                    <div>
                                        <p className="font-black text-gray-800 text-base">Rozeslat na jednotlivé adresy</p>
                                        <p className="text-xs text-gray-400 font-bold mt-1">+100 Kč / ks (adresy nám pak pošlete mailem)</p>
                                    </div>
                                </label>
                            </section>
                        )}

                        {/* STATICKÉ TLAČÍTKO VLOŽIT DO KOŠÍKU - ZVĚTŠENÝ TEXT */}
                        <div className="pt-6">
                            <button onClick={handleAddToCart} disabled={isAdded || uploading} className={`w-full py-6 rounded-2xl text-white font-black text-2xl uppercase tracking-widest transition-all shadow-xl active:scale-95 ${isAdded ? 'bg-green-500' : 'bg-brand-pink hover:opacity-95 disabled:grayscale'}`}>
            ...
            {/* FIXNÍ LIŠTA - ZVĚTŠENÝ TEXT */}
            <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-100 p-4 pb-10 z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
                <div className="max-w-7xl mx-auto flex flex-col items-center px-4">
                    <div className="mb-4 text-center">
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">CELKEM K ÚHRADĚ:</span>
                        <span className="text-4xl font-black text-brand-purple">{formatPrice(finalTotal)} Kč</span>
                    </div>
                    <button onClick={handleAddToCart} disabled={isAdded || uploading} className={`w-full max-w-lg py-6 rounded-2xl text-white font-black text-xl uppercase tracking-widest transition-all shadow-xl active:scale-95 ${isAdded ? 'bg-green-500' : 'bg-brand-pink hover:opacity-95 disabled:grayscale'}`}>
