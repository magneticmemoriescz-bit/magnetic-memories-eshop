...
const CheckoutPage: React.FC = () => {
    ...
    const sendEmailNotifications = async (order: OrderDetails) => {
        ...
        // Marketing consent info for Admin
        const marketingConsentHtml = order.marketingConsent 
            ? `<div style="margin-top: 20px; padding: 10px; background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; border-radius: 4px; font-weight: bold;">✅ Zákazník SOUHLASÍ se zveřejněním produktů pro reklamní účely.</div>`
            : `<div style="margin-top: 20px; padding: 10px; background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; border-radius: 4px;">❌ Zákazník NESOUHLASÍ se zveřejněním produktů.</div>`;
        ...
        const customerParams = {
            ...
            reply_to: "info@magneticmemories.cz",
        ...
        const ownerParams = {
            email: 'magnetic.memories@magnetify.cz', // Admin email for the {{email}} variable in template
        ...
        // ATTEMPT 1: Send Owner Email (Admin Notification)
        // Wrapped in try/catch so if the template config is missing "To Email", it doesn't block the customer order.
        try {
             await window.emailjs.send('service_8dmx38z', 'template_8ax2a2w', ownerParams);
        } catch (error) {
            console.error("Warning: Failed to send Admin notification email. Check EmailJS 'To Email' configuration.", error);
            // We continue despite error to ensure customer gets their confirmation and order is logged via webhook.
        }
        
        // ATTEMPT 2: Send Customer Email (Order Confirmation)
        // This is critical for user experience, so we let it throw if it fails (unless it's a minor network blip, but usually implies bad email)
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Validation for T&C
        if (!agreedToTerms) errors.terms = 'Pro dokončení objednávky musíte souhlasit s obchodními podmínkami.';
        ...
        if (Object.keys(errors).length === 0) {
            ...
            try {
                // Trigger webhook first (non-blocking) to ensure data is sent even if email fails later
                ...
                setSubmitError(`Odeslání objednávky se nezdařilo. Zkuste to prosím znovu. (Chyba: ${error.text || 'Email service error'})`);
            } finally {
                    ...
                    <section aria-labelledby="cart-heading" className="lg:col-span-7 bg-white p-8 rounded-lg shadow-lg">
                        ...
                        <div className="mt-8">
                            ...
                            <div className="space-y-4">
                               <RadioCard name="shipping" value="zasilkovna" checked={shippingMethod === 'zasilkovna'} onChange={() => { setShippingMethod('zasilkovna'); setFormErrors(p => ({...p, shipping: ''})) }} title="Zásilkovna - Výdejní místo" price={`${formatPrice(shippingCosts.zasilkovna)} Kč`} />
                                ...
                                <RadioCard name="shipping" value="posta" checked={shippingMethod === 'posta'} onChange={() => { setShippingMethod('posta'); setFormErrors(p => ({...p, shipping: ''})) }} title="Česká pošta - Balík Do ruky" price={`${formatPrice(shippingCosts.posta)} Kč`} />
                        ...
                        <div className="mt-8">
                             ...
                             <div className="space-y-4">
                                ...
                                <RadioCard name="payment" value="dobirka" checked={paymentMethod === 'dobirka'} onChange={() => { setPaymentMethod('dobirka'); setFormErrors(p => ({...p, payment: ''})) }} title="Na dobírku" price={`${formatPrice(paymentCosts.dobirka)} Kč`} />
                        ...
                        <div className="mt-6 space-y-4 pt-6 border-t border-gray-200">
                            <div className="relative flex items-start">
                                ...
                                <div className="ml-3 text-sm">
                                    <label htmlFor="terms" className="font-medium text-gray-700">
                                        Souhlasím s <Link to="/obchodni-podminky" target="_blank" className="text-brand-purple hover:underline">obchodními podmínkami</Link> <span className="text-red-500">*</span>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div className="relative flex items-start">
                                ...
                                <div className="ml-3 text-sm">
                                    ...
                                    <p className="text-gray-500 text-xs mt-1">Vaše fotky mohou být použity na našem Instagramu nebo webu jako ukázka naší práce.</p>
                                </div>
                            </div>
                        </div>
